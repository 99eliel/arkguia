(function(){
  ...
  loadScript('./crafting-data.js?v=271').then(()=>loadScript('./crafting.js?v=271')).catch(()=>{});
  ...
  const aliases={
    'Therizinosaur':'Therizinosaur','Spino':'Spinosaurus','Dire Bear':'Dire Bear','Dung Beetle':'Dung Beetle','Snow Owl':'Snow Owl','Rock Drake':'Rock Drake','Roll Rat':'Roll Rat','Anglerfish':'Anglerfish','Sabertooth Salmon':'Salmon','Woolly Rhino':'Woolly Rhino'
  };
  function current(){const shown=nameEl.textContent.trim();return(window.ARK_CREATURES||[]).find(c=>c.nome===shown)||null;}
  function fileCandidates(c){
    const base=aliases[c.original]||c.original||c.nome;
    const list=[];
    if(c.variante==='Aberrante'){
      list.push(`Aberrant ${base}_ASA.png`);
      list.push(`Aberrant ${base}.png`);
    }
    list.push(`${base}_ASA.png`);
    list.push(`${base}.png`);
    if(c.nome&&c.nome!==base){list.push(`${c.nome}_ASA.png`);list.push(`${c.nome}.png`);}
  ...
  function render(){
    ...
    const id=c.id||c.nome;if(id===last&&box.querySelector('img'))return;last=id;
    const my=++run,files=fileCandidates(c);let i=0;
    box.innerHTML='<div class="creaturePhotoState">🦖 Carregando imagem de '+c.nome+'...</div>';
    function next(){
      if(my!==run)return;
      if(i>=files.length){box.innerHTML='<div class="creaturePhotoState">🦖 Ainda não há imagem compatível cadastrada para esta criatura.</div>';return;}
      const file=files[i++];const img=new Image();img.alt='Imagem de '+c.nome;img.loading='eager';img.decoding='async';
      img.onload=()=>{if(my!==run)return;box.innerHTML='';box.appendChild(img);const credit=document.createElement('span');credit.className='creaturePhotoCredit';credit.textContent='Imagem: arkutils/species-images';box.appendChild(credit);};
      img.onerror=next;img.src=rawUrl(file);
    }
    next();
  }
