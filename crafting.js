...
function filteredRecipes(){const q=norm(search.value),cat=categorySelect.value;return recipes.filter(r=>{if(cat!=='all'&&r.categoria!==cat)return false;if(!q)return true;const aliases=Array.isArray(r.ptbrAliases)?r.ptbrAliases.join(' '):'';return norm(`${r.nome} ${r.original} ${aliases}`).includes(q);});}
...
function ingredientValues(name,baseRaw){const p=readPlan(current.id),need=(Number(baseRaw)||0)*p.quantity,have=Math.max(0,Number(p.owned[name])||0),missing=Math.max(0,need-have);return{need,have,missing};}
function updateIngredientLine(row,name,baseRaw){const values=ingredientValues(name,baseRaw),status=row.querySelector('.ingredientNeed');if(!status)return;status.className='ingredientNeed '+(values.missing?'miss':'ok');status.textContent=`Precisa ${formatQty(values.need)} • ${values.missing?'Faltam '+formatQty(values.missing):'✓ quantidade suficiente'}`;}
function updateCraftSummary(){if(!current)return;const p=readPlan(current.id);let complete=0;for(const[name,baseRaw]of current.ingredientes){const need=(Number(baseRaw)||0)*p.quantity,have=Math.max(0,Number(p.owned[name])||0);if(have>=need)complete++;}const summary=$('#craftSummary'),ready=complete===current.ingredientes.length;summary.className='craftSummary'+(ready?' ready':'');summary.innerHTML=ready?'<b>✅ Você já tem todos os materiais</b><div class="meta">Pode fabricar a quantidade planejada.</div>':`<b>🧭 Ainda falta material</b><div class="meta">${complete} de ${current.ingredientes.length} tipos de material completos.</div>`;}
function renderIngredients(){if(!current)return;const p=readPlan(current.id),box=$('#craftIngredients');box.innerHTML='<b>📦 Materiais</b>';for(const[name,baseRaw]of current.ingredientes){const values=ingredientValues(name,baseRaw),row=document.createElement('div');row.className='ingredientRow';row.innerHTML=`<div><strong>${name}</strong><div class="ingredientNeed ${values.missing?'miss':'ok'}">Precisa ${formatQty(values.need)} • ${values.missing?'Faltam '+formatQty(values.missing):'✓ quantidade suficiente'}</div></div><input class="ownedInput" type="number" min="0" step="1" inputmode="numeric" value="${values.have}" aria-label="Quantidade de ${name} que já tenho">`;const input=row.querySelector('input');input.oninput=()=>{const live=editPlan(current.id);live.owned[name]=Math.max(0,Number(input.value)||0);saveState();updateIngredientLine(row,name,baseRaw);updateCraftSummary();};input.onblur=()=>{if(input.value==='')input.value='0';};box.appendChild(row);}updateCraftSummary();}
