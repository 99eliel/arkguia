...
function renderUser(user){const registered=!!(user&&!user.isAnonymous);btn.classList.toggle('registered',registered);btn.textContent=registered?'✓ '+(user.displayName||user.email||'Conta'):'👤 Entrar';$('#authGuest').style.display=registered?'none':'block';$('#authAccount').style.display=registered?'block':'none';if(registered){$('#authTitle').textContent='Sua conta';$('#authAccountName').textContent=user.displayName||'Conta ArkGuia';$('#authAccountEmail').textContent=user.email||'Conta conectada';}else{$('#authTitle').textContent='Salvar seu progresso';}}
...
$('#authSignOut').onclick=async()=>{
  ...
  try{
    ...
    await auth.signOut();
    msg.textContent='';dlg.close();location.reload();
